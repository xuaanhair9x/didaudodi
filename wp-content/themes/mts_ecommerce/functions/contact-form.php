...
if ( !class_exists('mtscontact') ) {
	class mtscontact {
		...
		public function register_scripts() {
			$consent_text = __('Please check the consent checkbox if you want to proceed.', MTS_THEME_TEXTDOMAIN );
			wp_register_script('mtscontact', get_template_directory_uri() . '/js/contact.js', true);
			wp_localize_script( 'mtscontact', 'mtscontact', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ), 'consent_text' => $consent_text ) );
		}
		...
		public function get_form() {
			...
			if (!$this->success) {
				...
				<div class="mtscontact_consent">
					...
					<label for="mtscontact_consent">'.__('Please tick to agree to be contacted.',  MTS_THEME_TEXTDOMAIN).'</label>
				</div>
