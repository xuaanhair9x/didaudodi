...
<head>
    ...
    <title><?php _e( 'Product Comparison', MTS_THEME_TEXTDOMAIN ) ?></title>
    ...
    <style type="text/css">
        ...
        ?>
        table.compare-list .remove td a, table.compare-list .remove td a:hover, table.compare-list .remove td a .remove, table.compare-list .stock td span { color: <?php echo $mts_options['mts_color_scheme']; ?>; }
        table.compare-list .remove td a:hover .remove,  table.compare-list .add-to-cart td a  { background: <?php echo $mts_options['mts_color_scheme']; ?>; }
        table.compare-list .add-to-cart td a:hover { background: <?php echo $mts_options['mts_color_scheme2']; ?>; }
        table.compare-list .stock td span.out-of-stock, table.compare-list tr.price th, table.compare-list tr.price td { color: <?php echo $mts_options['mts_color_scheme3']; ?>; }
    </style>
</head>
<!-- END HEAD -->
...
<h1>
    ...
    <?php if ( ! $is_iframe ) : ?><a class="close" href="#"><?php _e( 'Close window [X]', MTS_THEME_TEXTDOMAIN ) ?></a><?php endif; ?>
</h1>
...
<table class="compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
    ...
    <?php if ( empty( $products ) ) : ?>
        ...
        <tr class="no-products">
            <td><?php _e( 'No products added in the compare table.', MTS_THEME_TEXTDOMAIN ) ?></td>
        </tr>
        ...
        <tr class="remove">
            ...
            <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->get_id() ?>
                <td class="<?php echo $product_class; ?>">
                    <a href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $product->get_id() ) ) ?>" data-product_id="<?php echo $product->get_id(); ?>"><?php _e( 'Remove', MTS_THEME_TEXTDOMAIN ) ?> <span class="remove">x</span></a>
                </td>
            <?php endforeach ?>
        </tr>
